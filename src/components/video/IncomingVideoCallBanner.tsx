...
export function IncomingVideoCallBanner() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("video_calls")
        .select("id,creator_id,subscriber_id,status,created_at,profiles:creator_id(display_name,username,avatar_url)")
  ...
  if (!call || open) {
    return <VideoCallPanel callId={open && call ? call.id : null} open={open && Boolean(call)} onClose={() => setOpen(false)} />;
  }
  ...
  return (
    <>
      <div className="fixed inset-x-3 bottom-20 z-[70] mx-auto flex max-w-lg items-center gap-3 rounded-2xl border border-violet-400/20 bg-neutral-950 p-3 text-white shadow-2xl lg:bottom-6 lg:left-auto lg:right-6 lg:inset-x-auto">
        <UserAvatar name={(call as any).profiles?.display_name} path={(call as any).profiles?.avatar_url} className="size-11" />
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold">Chamada de vídeo recebida</p>
          <p className="truncate text-xs text-white/60">{(call as any).profiles?.display_name ?? "Seu criador"} está chamando você.</p>
        </div>
