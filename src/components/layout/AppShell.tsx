...
export function AppShell({ children, title }: { children: ReactNode; title?: string }) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#07080d] text-foreground">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-white/[0.07] bg-[#090a10]/95 backdrop-blur-2xl lg:flex">
        <div className="flex items-center border-b border-white/[0.07] px-5 py-5">
        ...
        <div className="px-4 pt-5">
          <div className="rounded-2xl border border-brand/10 bg-gradient-to-br from-brand/[0.08] to-white/[0.015] px-4 py-3 shadow-[0_16px_40px_-28px_rgba(184,76,255,0.55)]">
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-brand/80">Espaço</p>
          ...
          {nav.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.to}
                className={cn(
                  "group relative flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium text-sidebar-foreground/65 transition-all duration-200 hover:bg-white/[0.035] hover:text-sidebar-foreground",
                  active && "bg-gradient-to-r from-brand/[0.14] to-white/[0.025] text-sidebar-foreground shadow-[inset_1px_0_0_rgba(184,76,255,0.9)]",
                )}
              >
        ...
        <div className="border-t border-white/[0.07] p-4">
          <div className="flex items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.018] p-3 shadow-lg">
            ...
            <Button variant="ghost" size="icon" aria-label={t("signOut", locale)} onClick={signOut} className="text-muted-foreground hover:text-foreground">
      ...
      <div className="relative lg:pl-64">
        ...
        <main className="mx-auto w-full max-w-7xl px-4 pb-28 pt-7 lg:px-8 lg:pb-12 lg:pt-8">{children}</main>
      </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-white/[0.07] bg-[#090a10]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-2xl lg:hidden">
        <div className="flex items-stretch justify-around px-2">
          {nav.slice(0, 5).map((item) => {
            const active = pathname === item.to || pathname.startsWith(`${item.to}/`);
            return (
              <Link key={item.to} to={item.to} className={cn("flex min-w-0 flex-1 flex-col items-center gap-1.5 py-2.5 text-[10px] font-medium text-muted-foreground", active && "text-brand")}>
