...
export function PayoutAdminPanel() {
  ...
  const q = useQuery({
    ...
    queryFn: async (): Promise<PayoutRequest[]> => {
      const { data, error } = await supabase
        ...
        .select(
          "id,creator_id,amount_cents,currency,destination,status,created_at,reviewed_at,rejection_reason,profiles:creator_id(display_name,username)",
        )
  ...
  if (q.isLoading) {
    return <div className="surface-card p-6">Carregando solicitações…</div>;
  }
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Revise solicitações dos criadores. Aprovar coloca o saque em processamento; nenhum
            pagamento externo é executado aqui.
          </p>
        </div>
      </div>
    ...
    <div className="surface-card p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
        <div className="min-w-0 flex-1">
          <p className="font-semibold">{payout.profiles?.display_name ?? "Criador"}</p>
          ...
          <p className="mt-2 text-xs">
            Status: <span className="font-medium">{payout.status}</span>
          </p>

          {payout.rejection_reason ? (
            <p className="mt-1 text-sm text-destructive">Motivo: {payout.rejection_reason}</p>
          ) : null}
        </div>
        ...
        {pending ? (
          <div className="w-full lg:max-w-sm">
            ...
            <div className="mt-2 flex gap-2">
              <Button className="flex-1 gap-2" onClick={() => onReview(payout.id, "approved")}>
              ...
              <Button
                ...
                onClick={() => onReview(payout.id, "rejected", reason)}
              >
                <X className="size-4" />
                Rejeitar
              </Button>
