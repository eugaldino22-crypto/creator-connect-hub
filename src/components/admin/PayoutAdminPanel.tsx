...
export function PayoutAdminPanel() {
  ...
  return <div className="space-y-4"><div className="flex items-center gap-2"><WalletCards className="size-5 text-primary" /><div><h2 className="text-xl font-semibold">Saques</h2><p className="text-sm text-muted-foreground">Revise solicitações dos criadores. Nenhum pagamento externo é executado aqui.</p></div></div>{q.data.length === 0 ? <div className="surface-card p-8 text-center text-sm text-muted-foreground">Nenhuma solicitação de saque.</div> : q.data.map((p: any) => <PayoutRow key={p.id} payout={p} onReview={review} />)}</div>;
}
