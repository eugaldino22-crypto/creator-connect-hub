...
export function FinanceDashboard() {
  ...
  const q = useQuery({
    queryKey: ["studio-finance", current?.user.id],
    enabled: Boolean(current?.user.id),
    queryFn: async () => {
      const [{ data: balance, error: balanceError }, { data: subs, error: subError }] =
        await Promise.all([
          supabase
          ...
          supabase
            .from("subscriptions")
            .select(
              "id,amount_cents,creator_amount_cents,currency,status,created_at,paid_at,profiles:subscriber_id(display_name,username)",
            )
            .eq("creator_id", current!.user.id)
    ...
    <div className="space-y-5">
      ...
      <div className="surface-card p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="font-semibold">Resumo de receitas</h3>

            <p className="text-xs text-muted-foreground">
              Baseado apenas em assinaturas confirmadas como ativas.
            </p>
          </div>
      ...
      <div className="surface-card overflow-hidden">
        <div className="border-b border-border p-5">
          <h3 className="font-semibold">Assinaturas recentes</h3>
        </div>

        {!q.data?.subs.length ? (
          <EmptyBlock
            title="Nenhuma assinatura"
            description="Quando houver assinaturas, elas aparecerão aqui."
          />
        ) : (
          <div className="divide-y divide-border">
            {q.data.subs.map((subscription) => (
              <div
                key={subscription.id}
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="font-medium">
                    {subscription.profiles?.display_name ?? "Assinante"}
                  </p>
                ...
                <div className="text-sm sm:text-right">
                  ...
                  <p className="text-xs text-muted-foreground">líquido · 15%</p>
                </div>
              </div>
            ))}
          </div>
