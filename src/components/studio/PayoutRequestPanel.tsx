...
export function PayoutRequestPanel({ availableCents = 0, onCreated }: { availableCents?: number; onCreated?: () => void }) {
 const {data:current}=useCurrentUser(); const [amount,setAmount]=useState(""); const [destination,setDestination]=useState(""); const [busy,setBusy]=useState(false); const [message,setMessage]=useState("");
 async function request(){ if(!current?.user.id)return; const cents=Math.round(Number(amount.replace(",","."))*100); if(!Number.isFinite(cents)||cents<=0||cents>availableCents){setMessage(`Informe um valor entre US$ 0,01 e ${formatCents(availableCents,"USD")}.`);return;} if(destination.trim().length<3){setMessage("Informe o destino do saque.");return;} setBusy(true);setMessage(""); const {error}=await supabase.rpc("request_creator_payout",{_amount_cents:cents,_currency:"USD",_destination:destination.trim()}); setBusy(false); if(error){setMessage(error.message);return;} setAmount("");setDestination("");setMessage("Solicitação enviada. O saque ficará pendente de análise administrativa.");onCreated?.(); }
