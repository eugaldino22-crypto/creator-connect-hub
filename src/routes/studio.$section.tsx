...
function SubscriberManager() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("subscriptions").select("id,subscriber_id,status,current_period_end,profiles:subscriber_id(username,display_name,avatar_url)").eq("creator_id", current!.user.id).eq("status", "active").order("created_at", { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
  });
  async function startCall(subscriberId: string) {
    setStarting(subscriberId); setCallError(null);
    try { const result = await createVideoCall(subscriberId); setCallId(result.call.id); setCallOpen(true); }
    catch (error) { setCallError(error instanceof Error ? error.message : "Não foi possível iniciar a chamada."); }
    finally { setStarting(null); }
  }
  return <AppShell title="Studio · Assinantes">
    <div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-semibold">Assinantes</h2><p className="mt-1 text-sm text-muted-foreground">Gerencie sua comunidade e inicie videochamadas individuais com assinantes ativos.</p></div><Users className="size-7 text-primary" /></div>
...
function CreatePost() {
  ...
  function addFiles(list: FileList | null) {
    ...
    setError(accepted.length !== list.length ? "Alguns arquivos foram ignorados. Imagens: até 15 MB; vídeos: até 200 MB." : "");
  }

  async function publish() {
    if (!data?.user.id || (!title.trim() && !body.trim() && files.length === 0)) return;
    setBusy(true); setError(""); setSaved(false);
    const { data: post, error: postError } = await supabase.from("posts").insert({ creator_id: data.user.id, title: title.trim() || null, body: body.trim() || null, visibility: exclusive ? "subscribers" : "public", is_published: true }).select("id").single();
    if (postError || !post) { setError(postError?.message ?? "Não foi possível criar a publicação."); setBusy(false); return; }
    const uploaded: { bucket: string; path: string; file: File; position: number }[] = [];
    try {
      for (const [position, file] of files.entries()) {
        const bucket = exclusive ? PREMIUM_BUCKET : PUBLIC_BUCKET;
        const path = await uploadUserFile({ bucket, userId: data.user.id, file, folder: `posts/${post.id}` });
        uploaded.push({ bucket, path, file, position });
      }
      if (uploaded.length) {
        const { error: mediaError } = await supabase.from("post_media").insert(uploaded.map((item) => ({ post_id: post.id, creator_id: data.user.id, bucket: item.bucket, storage_path: item.path, media_type: item.file.type.startsWith("video/") ? "video" : "image", is_private: exclusive, position: item.position })));
        if (mediaError) throw mediaError;
      }
      setTitle(""); setBody(""); setFiles([]); setExclusive(false); setSaved(true);
    } catch (err) {
      if (uploaded.length) await Promise.all(uploaded.map((item) => supabase.storage.from(item.bucket).remove([item.path])));
  ...
  return <AppShell title="Nova publicação"><div className="max-w-2xl space-y-4">
    <Input placeholder="Título" value={title} onChange={e=>setTitle(e.target.value)} />
    <Textarea placeholder="Escreva para sua comunidade…" className="min-h-40" value={body} onChange={e=>setBody(e.target.value)} />
    <label className="flex cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed border-border p-6 text-sm text-muted-foreground hover:bg-secondary/40"><Upload className="size-4" />Adicionar fotos ou vídeos<input className="sr-only" type="file" accept="image/*,video/*" multiple onChange={e=>addFiles(e.target.files)} /></label>
    {files.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{files.map((file,index)=><div key={`${file.name}-${index}`} className="relative overflow-hidden rounded-xl border border-border bg-secondary/30 p-2"><div className="flex aspect-square items-center justify-center overflow-hidden rounded-lg bg-background">{file.type.startsWith("image/") ? <img src={URL.createObjectURL(file)} alt="" className="size-full object-cover" /> : <ImageIcon className="size-8 text-muted-foreground" />}</div><p className="mt-2 truncate text-xs">{file.name}</p><button type="button" className="absolute right-2 top-2 rounded-full bg-background/90 p-1" onClick={()=>setFiles(files.filter((_,i)=>i!==index))}><X className="size-3"/></button></div>)}</div> : null}
    <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={exclusive} onChange={e=>setExclusive(e.target.checked)} /> Somente assinantes</label>
    {error ? <p className="text-sm text-destructive">{error}</p> : null}
    <Button disabled={busy} onClick={publish}>{busy ? "Enviando…" : "Publicar"}</Button>
    {saved && <p className="text-sm text-muted-foreground">Publicação criada com sucesso.</p>}
  </div></AppShell>;
}
